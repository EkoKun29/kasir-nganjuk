...
@section('content')
    <div class="card-body">
        ...
        @include('components.button-back', [
            'label' => 'Back',
            'url' => '/report-penjualan-cash',
        ])
        <form action="{{ route('penjualan-cash.update-induk', $head->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-4">
        ...
        <table class="table table-bordered table-hover bg-white">
            ...
            <tbody>
                @foreach ($data as $item)
                    <form action="{{ route('report-penjualan-cash.update', $item->id) }}" method="post">
                        @csrf
                        <tr>
